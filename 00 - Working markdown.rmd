...
## Gun crimes for all cities

```{r}
# source("~scripts/11 - Read crime data.R")
gun_crimes_df <- readRDS("~outputs/10/11_gun_crimes_df.rds")
```
...
```{r, eval = FALSE}
set.seed(1)
gun_crimes_sample_df <- sample_n(gun_crimes_df, 10000) 
gun_crimes_sample_ls <- gun_crimes_sample_df %>% 
...
```{r}
unique(gun_crimes_df$city)
unique(gun_crimes_df$state)
```
...
```{r, eval = FALSE}
plan(multiprocess)
gun_crimes_sample_ls <- future_map(gun_crimes_sample_ls,
                        ~ .x %>% 
...
```{r}
# source("~scripts/11 - Read crime data.R")
gun_crimes_clean <- readRDS("~outputs/20/21_gun_crimes_clean.rds")
```

```{r}
range(gun_crimes_clean$clean_occur_date, na.rm = TRUE)
range(gun_crimes_clean$clean_report_date, na.rm = TRUE)
```

Benchmarking results show `furrr` is fastest for manipulating the data.
```{r, eval = FALSE}
benchmark_results <- benchmark("flat" = {
  flat <- gun_crimes_sample_df %>% 
  mutate(clean_occur_date = anydate(occurdate), # use built-in formats from anytime package
         # correct some incorrectly parsed observations
         clean_occur_date = case_when(occurdate == "1" ~ as.Date(NA),
                                      is.na(clean_occur_date) & 
...
"purrr" = {map(gun_crimes_sample_ls,
                        ~ .x %>% 
                          mutate(clean_occur_date = anydate(occurdate), # use built-in formats from anytime package
         # correct some incorrectly parsed observations
         clean_occur_date = case_when(occurdate == "1" ~ as.Date(NA),
...
# plan(multiprocess)
"furrr" = {future_map(gun_crimes_sample_ls,
                        ~ .x %>% 
                          mutate(clean_occur_date = anydate(occurdate), # use built-in formats from anytime package
         # correct some incorrectly parsed observations
         clean_occur_date = case_when(occurdate == "1" ~ as.Date(NA),
