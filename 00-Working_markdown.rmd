...
```{r}
paste("There are",
      sum(is.na(guns_list$`San Francisco`$lon)), "NA longitudes and",
      sum(is.na(guns_list$`San Francisco`$lat)), "NA latitudes among",
      nrow(guns_list$`San Francisco`), "total incidents in San Francisco.")
```
...
```{r, results='asis', echo = FALSE, fig.height = 8}
for (i in 1:length(ggmap_list)) {
  cat("###### ",l$cityNames[i],"\n")
  print(ggmap_list[[i]])
  cat('\n\n')
}
