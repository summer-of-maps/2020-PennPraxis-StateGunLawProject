...
Read in the study area geographies w/ crime counts per tract
...
Plot of Moran's I for **census tracts and block groups selected by census-designated place**.
...
I_wide <- I_tmp %>% 
  pivot_wider(names_from = "geo",
              values_from = c("crime_I", "pop_I", "pval_crime", "pval_pop")) %>% 
  dplyr::select(City, 
...
Question for Dana: how might we interpret a city that has a large difference in Moran's I between tracts and block groups (e.g. Kansas City)?

Need to add columns comparing this to population.
  ...
  mutate(crime_tr_p = cell_spec(crime_tr_p,
         ...
         pop_BG_p = cell_spec(pop_BG_p,
                           ...
                           background = ifelse(str_detect(pop_BG_p, ">"),
                             "red", 
                             "white"))) %>% 
  arrange(desc(crime_tr)) %>%
  kable(format = "html",
        escape = FALSE,
        digits = 2,
        caption = "Moran's I for Crimes and Population by Tract and Block Group") %>%
...
tmp <- list.files("~outputs/Plots/33a_tract_crime_maps/byPlace",
                  full.names = TRUE)
